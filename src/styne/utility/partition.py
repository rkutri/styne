...
class PartitionDeprecated:
    """
    Index-based adapter for splitting and merging parameter coordinates.

    Operates on the coordinate arrays of arbitrary Parameter objects, with
    no requirement to store or own a specific parameter instance.
    """

    def __init__(self, partitionRule: List[List[int]]):
        """
        Parameters
        ----------
        partitionRule : List[List[int]]
            List of index lists. The i-th entry contains the coordinate
            indices belonging to partition component i. Together the lists
            must cover all indices [0, n-1] exactly once (no gaps, no
            overlaps). Index order within each sublist is preserved.
        """
        self._partition = partitionRule

    def component(self, idx: int, parameter: Parameter) -> ndarray:
        """
        Coordinate entries of parameter belonging to partition component idx.

        Uses numpy fancy indexing — order within each index list is preserved.
        """
        return parameter.coordinate[self._partition[idx]]

    def merge(self, components: List[ndarray]) -> ndarray:
        """
        Reconstruct a full coordinate array from the list of block arrays.

        Each component is placed back at its original indices. The partition
        must cover the full index range without gaps.
        """
        if len(components) != len(self._partition):
            raise ValueError(
                f"Expected {len(self._partition)} components, got {len(components)}."
            )
        totalDim = sum(len(idx) for idx in self._partition)
        result = np.empty(totalDim)
        for indices, comp in zip(self._partition, components):
            result[indices] = comp
        return result
